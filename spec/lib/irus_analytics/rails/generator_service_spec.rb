...
RSpec.describe GeneratorService do
  ...
  describe ".already_includes?" do
    let(:helper) { described_class.new(generator: generator,
  ...
  describe ".already_matches?" do
    ...
    it "it does not find non-existent line" do
      expect( helper.already_matches?( path_to_test_file,
                                       /#{Regexp.escape code_class_declaration_missing}/ ) ).to eq false
    end

    it "it does not find in non-existent file" do
      expect( helper.already_matches?( path_to_non_existent_file,
                                       /#{Regexp.escape code_class_declaration2}/ ) ).to eq false
    end
  ...
  describe ".inject_after", skip: !(defined? Rails::Generators::Base) do
    let(:helper) { described_class.new(generator: generator,
                                       generator_name: generator_name,
                                       debug_verbose: debug_verbose,
                                       cli_options: cli_options )}
